...
| **Distribuidora** | compras, retornables, almacenes, pedidos, salidas entre sucursales, **rutas de reparto** (chofer, cobranza, crédito, canastillas, convenios) y conteos, sin etiquetadora. |
...
|---|---|---|
| **Compras** | Proveedores, recepción de mercancía escaneando el código del proveedor, la factura del proveedor capturada después con sus renglones, cuentas por pagar con vencimientos, y pagos que salen de la gaveta abierta como retiro. Facturado contra recibido, por producto, lado a lado. | La factura es lo único que crea deuda y el único sitio donde vive el precio de compra; el inventario nunca lleva costo. Un solo camino de pago: el efectivo sale de la gaveta, el libro solo se agrega, anular compensa. Sin órdenes de compra. |
| **Retornables** | Canastillas, tarimas y totes que el proveedor deja en el negocio: libro por proveedor alimentado por la recepción, devoluciones y ajustes con motivo. Módulo aparte de compras (lo necesita) y de las canastillas de clientes de las rutas. | Libro solo-inserción; no se devuelve lo que no se debe. |
| **Almacenes** | Una sucursal tipo *almacén* (frigorífico rentado o bodega) solo guarda a granel: sin caja, sin etiquetas, sin conteos. Los traspasos a granel mueven kilos y cajas entre sucursales sin escanear, sale y entra en una sola transacción, con folio TG. | Un camión de cinco mil cajas no se escanea. De un almacén la mercancía solo va a la matriz. Lo etiquetado que se manda a un almacén vuelve al granel: sus etiquetas se dan de baja con el folio como motivo. A una tienda lo etiquetado viaja solo con salida escaneando. |
