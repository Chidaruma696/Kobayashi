...
| **Distributor** | purchasing, returnables, warehouses, orders, transfers between branches, **delivery routes** (driver, collections, credit, crates, agreements) and counts, without the labeler. |
...
|---|---|---|
| **Purchasing** | Suppliers, goods receipt by scanning the supplier's barcode, the supplier's invoice entered afterwards with its lines, accounts payable with due dates, and payments that leave the open drawer as a withdrawal. Invoiced vs received, per product, side by side. | The invoice is the only thing that creates debt and the only place a purchase price lives; stock never carries cost. One payment path: cash comes out of the drawer, the ledger is append-only, voiding compensates. No purchase orders. |
| **Returnables** | Crates, pallets and totes the supplier leaves with the business: ledger per supplier fed by the receipt, returns and adjustments with a reason. A separate module from purchasing (needs it) and from the customer crates of the routes. | Append-only ledger; nothing returned that is not owed. |
| **Warehouses** | A branch of type *warehouse* (a rented cold store or depot) only holds goods in bulk: no till, no labels, no counts. Bulk transfers move kilos and boxes between branches without scanning, in and out in one transaction, with a TG number. | A truck of five thousand boxes is not scanned. From a warehouse goods only go to the headquarters. Labelled goods sent to a warehouse go back to bulk: their labels are retired with the transfer number as the reason. To a shop, labelled goods travel only by scanned dispatch. |
