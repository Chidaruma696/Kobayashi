...
class MovimientoCredito < ApplicationRecord
  ...
  validate { errors.add(:monto_centavos, "un cargo suma") if tipo == "cargo" && monto_centavos.to_i <= 0 }
  validate { errors.add(:monto_centavos, "un abono o devolución resta") if %w[abono devolucion].include?(tipo) && monto_centavos.to_i >= 0 }
